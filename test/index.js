...
var remove    = require('es5-ext/array/#/remove')
  ...
  , filterMap = require('./filter-map-subset').tests
  , oToSet    = require('./to-set').tests;

module.exports = function (ObservableMap, a) {
	...
	map.clear();
	a.deep(toArray(map), [], "Clear: value");
	a.deep([sets, deletes, clears, others], [2, 1, 1, 0], "Clear: event");

	map.clear();
	a.deep(toArray(map), [], "Clear on empty: value");
	a.deep([sets, deletes, clears, others], [2, 1, 1, 0],
		"Clear on empty: event");

	filterMap(ObservableMap, a);
	oToSet(ObservableMap, a);
};
